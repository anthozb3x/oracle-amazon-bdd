...
BEGIN
    OPEN p_utilisateur_cursor FOR
        SELECT *
        FROM "AMAZONDATABSE"."UTILISATEUR"
...
AS
BEGIN
    INSERT INTO "AMAZONDATABSE"."UTILISATEUR" (
        "UTILISATEUR_UID",
        "UTILISATEUR_ID",
        "NOM",
        "PRENOM",
...
AS
BEGIN
    UPDATE "AMAZONDATABSE"."UTILISATEUR"
    SET
...
AS
BEGIN
    OPEN p_adresse_cursor FOR
        SELECT *
        FROM ADRESSE
        WHERE ADRESSE_UID = p_adresse_uid;
END get_adresse;
/
...
CREATE OR REPLACE PROCEDURE delete_commande (
    p_commande_uid IN VARCHAR2
)
    IS
BEGIN
    DELETE FROM COMMANDE WHERE COMMANDE_UID = p_commande_uid;
    COMMIT;
END delete_commande;
/
...
CREATE OR REPLACE FUNCTION get_commande (
    p_commande_uid IN VARCHAR2,
    p_commande_cursor OUT SYS_REFCURSOR
)
    RETURN SYS_REFCURSOR AS
BEGIN
    OPEN p_commande_cursor FOR
        SELECT *
        FROM COMMANDE
        WHERE COMMANDE_UID = p_commande_uid;
...
BEGIN
    insert_adresse('1', 'Adresse1', '123 Rue de la Paix', 1, SYSDATE, NULL);
    insert_adresse('2', 'Adresse2', '456 Avenue des Champs-Élysées', 0, SYSDATE, NULL);
    insert_adresse('3', 'Adresse3', '789 Boulevard Saint-Germain', 1, SYSDATE, NULL);
END;
/

-- Insertion des commandes
BEGIN
    insert_commande('1', SYSDATE, '1', '2', '1', '2');
